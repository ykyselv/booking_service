...
class ClientlistTest(APITestCase):
    ...
    def setUp(self):
        ...
        Schedule.objects.create(start_appointment=start_schedule_1, end_appointment=end_schedule_1)

        # 2nd instance of Schedule
        start_schedule_2 = datetime(2022, 7, 6, 12, 0, 0)
        end_schedule_2 = datetime(2022, 7, 6, 19, 0, 0)
        Schedule.objects.create(start_appointment=start_schedule_2, end_appointment=end_schedule_2)

        # 3rd instance of Schedule
        start_schedule_3 = datetime(2022, 7, 7, 9, 0, 0)
        end_schedule_3 = datetime(2022, 7, 7, 15, 0, 0)
        Schedule.objects.create(start_appointment=start_schedule_3, end_appointment=end_schedule_3)
        ...
        Specialist.objects.create(specialization='massagist', name='Prykhodko Mykola Ivanovych', schedule_id=1,
                                  location_id=1).save()

        Specialist.objects.create(specialization='endocrynologist', name='Vakulenko Mykola Stepanovych', schedule_id=2,
                                  location_id=2)

        Specialist.objects.create(specialization='dentist', name='Vakulenko Olga Ivanivna', schedule_id=3,
                                  location_id=3)
    ...
    def test_create_appointment_invalid_1(self):
        ...
        response = self.client.post(reverse('makeappoint'), data=data)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual('К сожалению, записаться на это время невозможно', response.json().get('post'))
